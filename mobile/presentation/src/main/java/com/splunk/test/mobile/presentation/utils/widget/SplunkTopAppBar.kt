...
/**
 * Vadzim Ramanovich's note:
 * ------------------------------------------------
 *
 * This custom TopAppBar implementation is based on the original Jetpack Compose TopAppBar,
 * but modified to remove unwanted color transition animations when changing themes.
 * This code causes the app bar color to transition with animation based on `overlappedFraction`.
 * However, when switching between light and dark themes, all other UI colors update instantly,
 * except for the TopAppBar, which creates an inconsistent visual experience.
 *
 * To fix this, I copied and modified the implementation from the following repository:
 * [CustomFlexibleTopAppBar](https://github.com/shivathapaa/SocialDemoProjects/blob/main/CustomTopAppBar/app/src/main/java/com/shivathapaa/customtopappbar/components/CustomFlexibleTopAppBar.kt)
 *
 * ------------------------------------------------
...
fun SplunkTopAppBar(
    ...
    val appBarContainerColor = colors.containerColor(fraction)
